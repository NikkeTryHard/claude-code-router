...
class ApiClient {
  ...
  // Query logs from GCli2API by request ID (for cross-application log correlation)
  async queryGcli2ApiLogs(
    gcli2apiUrl: string,
    token: string,
    reqId: string
  ): Promise<{ logs: any[]; total: number; limited: boolean }> {
    const url = `${gcli2apiUrl}/auth/logs/query?reqId=${encodeURIComponent(reqId)}`;
    ...
    if (!response.ok) {
      if (response.status === 401) {
        throw new Error('GCLI2API authentication failed');
      }
      throw new Error(`GCLI2API request failed: ${response.status}`);
    }
